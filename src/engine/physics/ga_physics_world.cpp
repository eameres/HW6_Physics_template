...
void ga_physics_world::step(ga_frame_params* params)
{
	...
	//test_intersections(params); // TODO: Homework 5.  Uncomment this line to test intersections!!
...
void ga_physics_world::step_linear_dynamics(ga_frame_params* params, ga_rigid_body* body)
{
	...
	// comment/uncomment this code if you'd like to see the brute forced version not for final solution though
	 ///*
	ga_vec3f acceleration_vector = overall_force.scale_result(1.0 / body->_mass);

	float dt = std::chrono::duration_cast<std::chrono::duration<float>>(params->_delta_time).count();
	
	acceleration_vector.scale(dt);

	body->add_linear_velocity(acceleration_vector.scale_result(dt));
	body->_transform.translate(body->_velocity.scale_result(dt));
	 //*/
